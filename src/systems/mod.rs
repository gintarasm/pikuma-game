...
use crate::{
    components::{RigidBodyComponent, TransformComponent, SpriteComponent},
    ecs::{entities::Entity, world::World, System, SystemAction, SystemBuilder}, logger::Logger,
};
...
impl SystemAction for MovementSystem {
    fn action(&mut self, world: &World, entities: &Vec<Entity>, delta_time: &Duration) {
        ...
        self.logger.info(&format!("Movement system updating with entities {}", entities.len()));
        ...
        for ent in entities {
            ...
            self.logger.info(&format!("Entity {} new position is now ({}, {})", ent.0, transform.position.x, transform.position.y));
        }
    }
